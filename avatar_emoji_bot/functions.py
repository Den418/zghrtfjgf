...
mask_image = Image.open(BASE_DIR / 'mask.png').convert('L')
fallback_emoji = '🟣'
...
async def _create_set(chat, user_id, emojis):

    title = get_set_title(chat.title)
    short_name = get_set_link(chat.id, client.me.username)

    request = stickers.CreateStickerSetRequest(
        user_id=user_id,
        title=title,
...
async def _create_emojis_from_profiles(chat):

    items = []

    photo = await client.download_profile_photo(chat, bytes)
    ...
    async for user in client.iter_participants(chat):
        ...
        photo = await client.download_profile_photo(user, bytes)
...
async def _create_emoji(original_photo: bytes, *, keywords: str = None):
    new_photo = _create_image(original_photo)

    file = await client.upload_file(new_photo)

    media = await client(UploadMediaRequest(
        tl_types.InputPeerSelf(),
        tl_types.InputMediaUploadedDocument(
            file=file,
            mime_type="image/webp",
            attributes=[
                tl_types.DocumentAttributeImageSize(100, 100),
                tl_types.DocumentAttributeFilename("emoji.webp")
            ]
        )
    ))

    document = get_input_document(media)

    return tl_types.InputStickerSetItem(
        document=document,
